...
func (s *Supplier) MergeFiles() error {
	...
	sourceFile1, err := os.OpenFile(filepath.Join(s.Stager.BuildDir(), "requirements.txt"), os.O_APPEND, 0644) 
	if err != nil {
		return err
	}
	...
	targetFile, err := os.OpenFile(filepath.Join(s.Stager.BuildDir(), "requirements-conda.txt"), os.O_APPEND|O_WRONLY, 0644)
  	if err != nil {
   		return err
 	}
